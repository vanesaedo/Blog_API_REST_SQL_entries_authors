-- Crear tabla authors
    ...
    -- Sacar los datos modificados segun el ejercicio 3
     SELECT e.title,e.content,e.date,e.category,a.name,a.surname,a.image
        FROM entries AS e
        INNER JOIN authors AS a
        ON e.id_author=a.id_author
    ...
    -- Actualizar los datos del ejercicio 3
    UPDATE entries
        SET
            ...
            date='2014-04-16', 
            category='Coches',
        id_author=(SELECT id_author FROM authors WHERE email = 'alejandro@thebridgeschool.es')
        WHERE title = 'El ferrari fake';
